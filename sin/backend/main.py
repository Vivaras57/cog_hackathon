...
def calculate_roi(prediction: int) -> tuple[float, float]:
    """Calculate ROI as in the original code"""
    hospitalization_cost = 5000
    intervention_cost = 200
    
    if prediction == 1:  # Predicted diabetic
        saved_cost = hospitalization_cost
        net_profit = saved_cost - intervention_cost
        roi_percent = (net_profit / intervention_cost) * 100
    else:
        net_profit = 0
        roi_percent = 0
...
async def startup_event():
    ...
    if success:
        ...
        print("⚠️  Model components not found. Using fallback prediction.")
...
async def predict_diabetes_risk(patient_data: PatientData):
    ...
    try:
        ...
        net_profit, roi_percent = calculate_roi(prediction)
...
async def get_model_info():
    ...
    return {
        ...
        "model_type": str(type(model).__name__) if model is not None else "fallback"
    }
