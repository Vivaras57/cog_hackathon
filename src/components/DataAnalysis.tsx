...
export default function DataAnalysis({ patients }: DataAnalysisProps) {
  ...
  const avgByAge = {
    young: patients.filter(p => p.age < 35).reduce((sum, p) => sum + p.finalScore, 0) / 
           Math.max(patients.filter(p => p.age < 35).length, 1),
    middle: patients.filter(p => p.age >= 35 && p.age < 55).reduce((sum, p) => sum + p.finalScore, 0) / 
            Math.max(patients.filter(p => p.age >= 35 && p.age < 55).length, 1),
    senior: patients.filter(p => p.age >= 55).reduce((sum, p) => sum + p.finalScore, 0) / 
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          <p className="text-sm text-gray-600 text-center">
            Distribution of {patients.length} patients across risk tiers. Higher tiers indicate greater intervention needs.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        <div className="flex items-center mb-6">
          ...
          <h3 className="text-xl font-semibold text-gray-900">Risk Factor Correlations</h3>
        </div>
            ...
            <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              <div className="flex-1">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm font-bold text-gray-900">{item.correlation.toFixed(2)}</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full ${
                      item.correlation > 0.6 ? 'bg-red-500' :
                      item.correlation > 0.4 ? 'bg-orange-500' :
                      item.correlation > 0.2 ? 'bg-yellow-500' : 'bg-green-500'
                    }`}
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        <div className="flex items-center mb-6">
          ...
          <h3 className="text-xl font-semibold text-gray-900">Monthly Risk Trends</h3>
        </div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    <div className="flex items-center">
                      ...
                      <span className={
                        ...
                        {index === 0 ? 'Baseline' : index === 1 ? 'Improving' : 'Increasing'}
                      </span>
                    </div>
                  </td>
                </tr>
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <h4 className="text-sm font-semibold text-blue-900 mb-2">Highest Risk Factors</h4>
